...
def register_rainbow_web(app, line_bot_api):
    ...
    async def admin_announcement_edit(announcement_id: int, request: Request):
        ...
        if not content: raise HTTPException(400,'請填寫公告內容。')
        if link_url and not re.match(r'^https?://',link_url,re.I): raise HTTPException(400,'公告連結必須以 http:// 或 https:// 開頭。')
        ...
        try:
            with conn.cursor() as c:
                if image_bytes is not None:
                    c.execute('UPDATE web_announcements SET title=%s,content=%s,link_url=%s,sort_order=%s,image_data=%s,image_mime=%s,is_active=1 WHERE id=%s AND group_id=%s',(title,content,link_url,sort_order,image_bytes,image_mime,announcement_id,gid))
                elif remove_image:
                    c.execute("UPDATE web_announcements SET title=%s,content=%s,link_url=%s,sort_order=%s,image_data=NULL,image_mime='',is_active=1 WHERE id=%s AND group_id=%s",(title,content,link_url,sort_order,announcement_id,gid))
                else:
                    c.execute('UPDATE web_announcements SET title=%s,content=%s,link_url=%s,sort_order=%s,is_active=1 WHERE id=%s AND group_id=%s',(title,content,link_url,sort_order,announcement_id,gid))
                if c.rowcount==0: raise HTTPException(404,'找不到公告。')
        ...
        push_ok,push_message=_push_announcement_to_line(announcement_id,gid,uid,line_bot_api)
        return {'ok':True,'id':announcement_id,'push_ok':push_ok,'push_message':push_message}
    ...
    async def announcement_image(announcement_id: int, request: Request):
        ...
        try:
            with conn.cursor() as c:
                c.execute('SELECT image_data,image_mime FROM web_announcements WHERE id=%s AND group_id=%s', (announcement_id, gid))
                row = c.fetchone() or {}
        finally:
            conn.close()
        data = row.get('image_data')
    ...
    async def admin_announcement(request: Request):
        ...
        try:
            with conn.cursor() as c:
                c.execute('INSERT INTO web_announcements(group_id,title,content,image_data,image_mime,link_url,created_by,sort_order,is_active) VALUES(%s,%s,%s,%s,%s,%s,%s,%s,1) RETURNING id',(gid,title,content,image_bytes,image_mime,link_url,uid,sort_order))
                inserted=c.fetchone() or {}
                announcement_id=int(inserted.get('id') or 0)
                c.execute('INSERT INTO web_admin_logs(group_id,user_id,action,detail) VALUES(%s,%s,%s,%s)',(gid,uid,'announcement_create',str(announcement_id)))
            conn.commit()
        finally: conn.close()
        push_ok,push_message=_push_announcement_to_line(announcement_id,gid,uid,line_bot_api)
        return {'ok':True,'has_image':bool(image_bytes),'id':announcement_id,'push_ok':push_ok,'push_message':push_message}
    ...
    def _push_announcement_to_line(announcement_id: int, gid: str, uid: str, api):
        conn=get_connection()
        try:
            with conn.cursor() as c:
                c.execute('SELECT id,title,content,link_url,(image_data IS NOT NULL) has_image FROM web_announcements WHERE id=%s AND group_id=%s AND is_active=1',(announcement_id,gid))
                row=c.fetchone()
                if not row: return False,'找不到啟用中的公告。'
                row=dict(row)
        finally: conn.close()
        try:
            from linebot.models import FlexSendMessage
            base=_public_base_url()
            view_url=make_player_entry_url(gid,f'/player?tab=announcements&announcement={announcement_id}') or base or 'https://line.me'
            bubble={'type':'bubble','size':'mega','styles':{'body':{'backgroundColor':'#120B38'},'footer':{'backgroundColor':'#120B38','separator':True,'separatorColor':'#4C3C88'}},'body':{'type':'box','layout':'vertical','paddingAll':'18px','spacing':'md','contents':[{'type':'text','text':'📢 RAINBOW LIFE','size':'xs','weight':'bold','color':'#C4B5FD'},{'type':'text','text':str(row.get('title') or 'Rainbow Life 公告')[:80],'size':'xl','weight':'bold','color':'#FFFFFF','wrap':True},{'type':'text','text':str(row.get('content') or '')[:700],'size':'sm','color':'#EDE9FE','wrap':True,'maxLines':10,'lineSpacing':'5px'}]},'footer':{'type':'box','layout':'vertical','paddingAll':'12px','contents':[{'type':'button','style':'primary','height':'sm','color':'#8B5CF6','action':{'type':'uri','label':'查看完整公告','uri':view_url}}]}}
            if row.get('has_image') and base:
                bubble['hero']={'type':'image','url':f"{base}/api/rainbow/public/announcement/{announcement_id}/image?gid={quote(str(gid),safe='')}",'size':'full','aspectRatio':'20:13','aspectMode':'cover','action':{'type':'uri','uri':view_url}}
            target_gid=_announcement_line_target(gid,uid)
            if not target_gid or target_gid=='PRIVATE':
                return False,'找不到可推播的 LINE 群組，請從群組內重新開啟後台。'
            api.push_message(target_gid,FlexSendMessage(alt_text='📢 Rainbow Life 公告',contents=bubble))
        except Exception as exc:
            return False,f'聊天室推播失敗：{exc}'
        conn=get_connection()
        try:
            with conn.cursor() as c:
    ...
    async def admin_announcement_push(announcement_id: int, request: Request):
        ...
        ok,message=_push_announcement_to_line(announcement_id,gid,uid,line_bot_api)
        if not ok: raise HTTPException(500,message)
