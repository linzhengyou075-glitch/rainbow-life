...
def ensure_web_tables():
    ...
    try:
        with conn.cursor() as c:
            ...
            c.execute("ALTER TABLE web_avatar_frames ADD COLUMN IF NOT EXISTS asset_name TEXT NOT NULL DEFAULT ''")
            ...
            frame_defs = [
                ('rainbow_basic','🌈 彩虹星光框',0,False,False,''),
                ('star_guard','✨ 星曜守護框',0,False,False,''),
                ('ice_crystal','❄️ 冰晶彩虹框',0,False,False,''),
                ('diamond_crown','💎 VIP鑽石框',0,True,False,'vip-frame.png'),
                ('leader_glory_wings','彩虹鑽石之翼 (Owner)',0,False,True,'frames/39557.png'),
            ]
            for frame in frame_defs:
                c.execute("""INSERT INTO web_avatar_frames
                    (frame_key,name,price,vip_only,owner_only,is_active,asset_name)
                    VALUES(%s,%s,%s,%s,%s,TRUE,%s)
                    ON CONFLICT(frame_key) DO UPDATE SET
                        name=EXCLUDED.name,price=0,vip_only=EXCLUDED.vip_only,
                        owner_only=EXCLUDED.owner_only,is_active=TRUE,asset_name=EXCLUDED.asset_name""", frame)
        conn.commit()
    finally:
        conn.close()
    ...
    try:
        with conn.cursor() as c:
            ...
            if role == 'owner':
                c.execute("""INSERT INTO web_user_frames
                    (group_id,user_id,frame_key,equipped,source,source_key,permanent,updated_at)
                    VALUES(%s,%s,'leader_glory_wings',FALSE,'owner','role',TRUE,CURRENT_TIMESTAMP)
...
def _player_data(line_bot_api, group_id, user_id):
    ...
    try:
        ...
        conn.close()
    if role == 'owner':
        equipped_frame = equipped_frame or 'leader_glory_wings'
    elif is_vip:
    ...
    if equipped_frame == 'leader_glory_wings' and role != 'owner':
        equipped_frame = 'rainbow_basic'
    for frame in frames:
...
function frameArtUrl(key){if(key==='leader_glory_wings')return '/rainbow-static/frames/39557.png?v=39557';if(key==='diamond_crown')return '/rainbow-static/vip-frame.png?v=2';return '/rainbow-static/member-frame.png?v=2'}function activeFrameKey(){if(!DATA)return'rainbow_basic';return DATA.role==='owner'?'leader_glory_wings':(DATA.vip?'diamond_crown':(DATA.equipped_frame||'rainbow_basic'))}function applyFrameArtwork(){if(!DATA)return;let key=activeFrameKey(),url=frameArtUrl(key);let wrap=document.getElementById('avatarWrap'),art=document.getElementById('avatarFrameArt');if(wrap&&art){wrap.className='avatar-wrap has-art frame-'+key;art.src=url;art.hidden=false}let shell=document.getElementById('frameAvatarShell'),preview=document.getElementById('framePreviewArt');if(shell&&preview){shell.className='frame-avatar-shell has-art frame-'+key;preview.src=url;preview.hidden=false}let compact=document.getElementById('s331AvatarArt');if(compact){compact.src=url;compact.hidden=false}let cardArt=document.getElementById('cardPreviewFrameArt'),cardHost=document.getElementById('cardPreviewFrame');if(cardArt&&cardHost){cardArt.src=url;cardArt.hidden=key!=='leader_glory_wings';cardHost.classList.toggle('owner-active',key==='leader_glory_wings')}document.querySelectorAll('[data-rainbow-avatar-frame]').forEach(function(host){let pic=host.querySelector('img:not(.owner-frame-overlay)'),overlay=host.querySelector('.owner-frame-overlay');if(!overlay){overlay=document.createElement('img');overlay.className='owner-frame-overlay';overlay.alt='';overlay.setAttribute('aria-hidden','true');host.appendChild(overlay)}overlay.src=url;overlay.hidden=key!=='leader_glory_wings';if(pic)pic.classList.add('owner-frame-photo')})}function observeAvatarFrames(){if(window.__rainbowFrameObserver)return;window.__rainbowFrameObserver=new MutationObserver(()=>{clearTimeout(window.__rainbowFrameTimer);window.__rainbowFrameTimer=setTimeout(applyFrameArtwork,60)});window.__rainbowFrameObserver.observe(document.body,{childList:true,subtree:true})}renderFrameVipCenter=function(){if(!DATA)return;let img=document.getElementById('framePreviewAvatar');if(img)img.src=DATA.picture_url||'/rainbow-static/rainbow_life_boy.png';let name=document.getElementById('framePreviewName');if(name)name.textContent=DATA.name||'Rainbow';let chip=document.getElementById('frameVipChip');if(chip){chip.textContent=DATA.vip?'💎 VIP '+(DATA.vip_until||'永久'):'一般會員';chip.classList.toggle('active',!!DATA.vip)}let grid=document.getElementById('frameShopGrid'),frames=DATA.frames||[];if(grid){grid.innerHTML=frames.map(function(f){let locked=!f.available,active=!!f.equipped,owned=!!f.owned;let source=f.vip_only?'VIP 專屬':f.owner_only?'Owner 專屬限定':(f.achievement_name||'成就／活動解鎖');let status=active?'使用中':locked?'權限限定':owned?'套用':'尚未解鎖';let disabled=locked||active||!owned;return '<article class="frame-option '+(active?'active ':'')+(disabled?'locked':'')+'"><div class="frame-option-top"><div><b>'+esc(f.name||frameLabel(f.frame_key))+'</b><small>不可購買・'+esc(source)+'</small></div><span class="frame-price">'+esc(source)+'</span></div><button class="step32-frame" data-key="'+esc(f.frame_key)+'" '+(disabled?'disabled':'')+'>'+status+'</button></article>';}).join('');grid.querySelectorAll('.step32-frame:not([disabled])').forEach(function(b){b.onclick=function(){equipFrameInline(b.dataset.key)}})}applyEquippedFrame();applyFrameArtwork()};
...
function step331Setup(){
  ...
  .s331-avatar-frame{position:relative;width:104px;height:104px;flex:0 0 104px;display:grid;place-items:center;filter:drop-shadow(0 0 14px rgba(153,84,255,.72))}.s331-avatar{position:absolute;width:66px;height:66px;border-radius:50%;object-fit:cover;border:0;box-shadow:none;z-index:1}.s331-avatar-art{position:absolute;inset:0;width:100%;height:100%;object-fit:contain;z-index:2;pointer-events:none;animation:ownerFramePulse 4.2s ease-in-out infinite}.s331-avatar-frame:after{content:"";position:absolute;inset:14px;border-radius:50%;box-shadow:0 0 18px rgba(168,91,255,.7),inset 0 0 14px rgba(103,168,255,.34);animation:ownerFrameGlow 3s ease-in-out infinite;z-index:0}@keyframes ownerFramePulse{50%{transform:scale(1.035);filter:brightness(1.18)}}@keyframes ownerFrameGlow{50%{box-shadow:0 0 28px rgba(196,106,255,.95),inset 0 0 20px rgba(103,168,255,.5)}}.s331-profile-main{min-width:0;flex:1}.s331-profile h2{font-size:20px;margin:0;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.s331-role{font-size:11px;color:#dcd2f8;margin-top:3px}.s331-profile-meta{display:flex;gap:6px;flex-wrap:wrap;margin-top:8px}.s331-chip{font-size:11px;padding:5px 8px;border-radius:999px;background:rgba(255,255,255,.09);border:1px solid rgba(255,255,255,.13)}.s331-exp{height:9px;border-radius:99px;background:rgba(4,2,27,.65);overflow:hidden;margin-top:9px}.s331-exp i{display:block;height:100%;background:linear-gradient(90deg,#68dfff,#9b79ff,#ff77cd,#ffe16e);transition:width .7s ease}
  ...
  @media(max-width:620px){#step331CompactHome{gap:10px}.s331-profile{padding:13px}.s331-avatar-frame{width:94px;height:94px;flex-basis:94px}.s331-avatar{width:60px;height:60px}.s331-tile{min-height:84px;padding:10px}.main{padding-bottom:90px}}
  `;
...
const _step331OldRenderPersonal=typeof renderPersonal==='function'?renderPersonal:null;if(_step331OldRenderPersonal)renderPersonal=function(){_step331OldRenderPersonal();step331Render();applyFrameArtwork();setTimeout(applyFrameArtwork,120)};document.addEventListener('DOMContentLoaded',()=>{observeAvatarFrames();setTimeout(applyFrameArtwork,250);setTimeout(applyFrameArtwork,900)});
setTimeout(step331Setup,0);setTimeout(step331Render,900);setTimeout(step352RestoreAdminSection,1100);

/* Phase 1 第三階段｜前台頭像框中心 */
(function(){
 const css=document.createElement('style');
 css.textContent='.p3fc{display:grid;gap:12px;text-align:left}.p3hero{display:grid;grid-template-columns:100px 1fr;gap:14px;align-items:center;padding:14px;border-radius:18px;background:linear-gradient(145deg,rgba(130,83,255,.22),rgba(255,113,204,.12));border:1px solid rgba(226,205,255,.25)}.p3preview,.p3thumb{position:relative;margin:auto}.p3preview{width:92px;height:92px}.p3thumb{width:70px;height:70px}.p3preview .base,.p3thumb .base{position:absolute;inset:17%;width:66%;height:66%;border-radius:50%;object-fit:cover}.p3preview .art,.p3thumb .art{position:absolute;inset:0;width:100%;height:100%;object-fit:contain;animation:p3float 3.4s ease-in-out infinite}.p3tabs{display:flex;gap:8px;overflow:auto}.p3tabs button{border:1px solid rgba(255,255,255,.14);background:rgba(255,255,255,.06);color:#ddd4f2;border-radius:999px;padding:8px 13px}.p3tabs button.active{background:linear-gradient(135deg,#8a5cff,#ec6fbd);color:#fff}.p3grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:9px;max-height:46vh;overflow:auto}.p3card{border-radius:16px;padding:10px;background:rgba(255,255,255,.055);border:1px solid rgba(255,255,255,.12);display:grid;gap:7px}.p3card.active{border-color:#f2c8ff;box-shadow:0 0 20px rgba(157,92,255,.2)}.p3card.locked{opacity:.62}.p3card small{color:#cfc5e7}.p3card button{border:0;border-radius:11px;padding:9px;background:linear-gradient(135deg,#8257ff,#e767bd);color:#fff;font-weight:700}.p3card button:disabled{background:rgba(255,255,255,.09);color:#aaa2bd}.p3note{font-size:12px;color:#cfc5e7;padding:10px 12px;border-radius:13px;background:rgba(255,255,255,.045)}@keyframes p3float{0%,100%{transform:translateY(0)}50%{transform:translateY(-2px)}}';
 document.head.appendChild(css);
 let filter='all';
 function source(f){if(f.owner_only)return 'Owner 專屬限定';if(f.vip_only)return 'VIP 有效期間';if(f.achievement_name)return f.achievement_name;return f.frame_key==='rainbow_basic'?'預設頭像框':'成就／活動取得'}
 function art(k){return frameArtUrl(k||'rainbow_basic')}
 window.p3Filter=function(v,b){filter=v;document.querySelectorAll('.p3tabs button').forEach(x=>x.classList.toggle('active',x===b));p3Render()};
 window.p3Render=function(){let box=document.getElementById('p3grid');if(!box||!DATA)return;let fs=(DATA.frames||[]).slice();if(filter==='owned')fs=fs.filter(f=>f.owned);if(filter==='locked')fs=fs.filter(f=>!f.owned);let av=DATA.picture_url||'/rainbow-static/rainbow_life_boy.png';box.innerHTML=fs.length?fs.map(f=>{let active=!!f.equipped,owned=!!f.owned,available=!!f.available,forced=DATA.role==='owner'||(DATA.vip&&f.vip_only),disabled=active||!owned||!available||forced,text=active?'使用中':!owned?'尚未取得':!available?'目前不可用':'使用';return '<article class="p3card '+(active?'active ':'')+((!owned||!available)?'locked':'')+'"><div class="p3thumb"><img class="base" src="'+esc(av)+'"><img class="art" src="'+esc(art(f.frame_key))+'"></div><b>'+esc(f.name||frameLabel(f.frame_key))+'</b><small>'+esc(source(f))+'</small><button '+(disabled?'disabled':'')+' onclick="p3Equip(\''+esc(f.frame_key)+'\')">'+text+'</button></article>'}).join(''):'<div class="p3note" style="grid-column:1/-1">這個分類目前沒有頭像框。</div>'};
 window.p3Equip=async function(k){try{let j=await api('/api/rainbow/frame/equip',{method:'POST',body:JSON.stringify({frame_key:k})});toast(j.message||'頭像框已套用');DATA=await api('/api/rainbow/me');if(typeof renderPersonal==='function')renderPersonal();if(typeof step331Render==='function')step331Render();p3Open()}catch(e){toast(e.message||'套用失敗')}};
 window.p3Open=function(){if(!DATA){toast('資料載入中');return}let k=DATA.equipped_frame||'rainbow_basic',cur=(DATA.frames||[]).find(f=>f.frame_key===k)||{},av=DATA.picture_url||'/rainbow-static/rainbow_life_boy.png',rule=DATA.role==='owner'?'群長固定使用最高限定框':DATA.vip?'VIP 有效期間自動使用 VIP 框':'可自由切換已取得的永久框';document.getElementById('dialog').innerHTML='<div class="p3fc"><div class="p3hero"><div class="p3preview"><img class="base" src="'+esc(av)+'"><img class="art" src="'+esc(art(k))+'"></div><div><b>🖼️ 我的頭像框</b><small style="display:block;margin-top:6px">目前配戴：'+esc(cur.name||frameLabel(k))+'</small><small style="display:block;margin-top:4px">'+esc(rule)+'</small></div></div><div class="p3tabs"><button class="active" onclick="p3Filter(\'all\',this)">全部</button><button onclick="p3Filter(\'owned\',this)">已取得</button><button onclick="p3Filter(\'locked\',this)">未取得</button></div><div class="p3grid" id="p3grid"></div><div class="p3note">頭像框無法購買。活動框與成就框取得後永久保留；VIP 到期會自動恢復原本的永久框。</div><button class="btn" onclick="closeModal()">關閉</button></div>';filter='all';document.getElementById('modal').classList.add('show');p3Render()};
 const oldAction=window.action;window.action=async function(n){if(n==='frame'){p3Open();return}return oldAction(n)};window.step331OpenFrames=function(){p3Open()};
})();
</script></body></html>'''
    ...
    async def web_equip_frame(request: Request):
        ...
        try:
            with conn.cursor() as c:
                ...
                if role=='owner' and key!='leader_glory_wings': raise HTTPException(403,'Owner 固定使用彩虹鑽石之翼專屬框。')
