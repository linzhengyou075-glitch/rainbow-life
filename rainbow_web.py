...
def ensure_web_tables():
    ...
    try:
        with conn.cursor() as c:
            ...
            for sql in (
                "ALTER TABLE web_announcements ADD COLUMN IF NOT EXISTS image_url TEXT NOT NULL DEFAULT ''",
                "ALTER TABLE web_announcements ADD COLUMN IF NOT EXISTS category TEXT NOT NULL DEFAULT '公告'",
                "ALTER TABLE web_announcements ADD COLUMN IF NOT EXISTS start_date TEXT NOT NULL DEFAULT ''",
                "ALTER TABLE web_announcements ADD COLUMN IF NOT EXISTS end_date TEXT NOT NULL DEFAULT ''",
                "ALTER TABLE web_announcements ADD COLUMN IF NOT EXISTS push_to_chat INTEGER NOT NULL DEFAULT 0",
                "ALTER TABLE web_announcements ADD COLUMN IF NOT EXISTS detail_type TEXT NOT NULL DEFAULT 'announcement'"
            ):
                c.execute(sql)
            ...
            c.execute('''CREATE TABLE IF NOT EXISTS web_personal_notifications (id BIGSERIAL PRIMARY KEY, group_id TEXT NOT NULL, user_id TEXT NOT NULL, kind TEXT NOT NULL DEFAULT 'system', title TEXT NOT NULL, content TEXT NOT NULL DEFAULT '', is_read INTEGER NOT NULL DEFAULT 0, created_at TIMESTAMPTZ DEFAULT CURRENT_TIMESTAMP)''')
            c.execute('CREATE INDEX IF NOT EXISTS idx_web_notify_user ON web_personal_notifications(group_id,user_id,is_read,created_at DESC)')
...
def _latest_sign_date(group_id, user_id):
    """Read the newest successful sign-in date across legacy sign_records schemas."""
    conn = get_connection()
    try:
        with conn.cursor() as c:
            for col in ('sign_date', 'date', 'created_at', 'signed_at'):
                try:
                    c.execute(f'SELECT MAX({col}) AS latest FROM sign_records WHERE group_id=%s AND user_id=%s', (group_id, user_id))
                    row = c.fetchone() or {}
...
def _player_data(line_bot_api, group_id, user_id):
    ...
    try:
        with conn.cursor() as c:
            ...
            c.execute("SELECT id,title,content,image_url,category,start_date,end_date,push_to_chat,detail_type,created_at FROM web_announcements WHERE group_id=%s AND is_active=1 AND (start_date='' OR start_date<=%s) AND (end_date='' OR end_date>=%s) ORDER BY id DESC LIMIT 10", (group_id, datetime.datetime.now(TZ).date().isoformat(), datetime.datetime.now(TZ).date().isoformat()))
            ...
            c.execute('SELECT id,kind,title,content,is_read,created_at FROM web_personal_notifications WHERE group_id=%s AND user_id=%s ORDER BY created_at DESC LIMIT 20', (group_id, user_id))
            notifications = [dict(x) for x in c.fetchall()]
    finally:
        conn.close()
    ...
    return {
        ...
        'total_sign': total_sign, 'latest_sign_date': _latest_sign_date(group_id, user_id), 'today_messages': int(p.get('today_msg_count') or 0),
...
/* v3.6.0 Package 1: fixed-size Rainbow Life profile card */
.profile-card{max-width:760px;margin:0 auto;border-radius:24px;overflow:hidden;min-height:430px}
.profile-card .hero{min-height:430px;height:430px;grid-template-columns:190px 1fr}
.profile-card .avatar{width:160px;height:160px}
.profile-meta{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:8px;margin-top:12px}
.profile-meta .mini{padding:10px;border:1px solid #4a3c95;border-radius:14px;background:#17103f;text-align:center;min-width:0}
.profile-meta .mini b{display:block;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
.latest-sign{margin-top:12px;padding:11px 14px;border-radius:14px;background:#0d0a2eaa;border:1px solid #6a55c0;text-align:center;font-weight:800}
.profile-only-button{display:block;width:100%;margin-top:12px;text-align:center;text-decoration:none}
.profile-card .stats{grid-template-columns:repeat(4,1fr)}
.profile-card .stat.coins-stat{display:none}
@media(max-width:760px){.profile-card{min-height:520px}.profile-card .hero{height:auto;min-height:520px}.profile-meta{grid-template-columns:repeat(3,1fr)}.profile-card .stats{grid-template-columns:repeat(2,1fr)}}

.notice-stack{display:grid;gap:10px}.notice-card{border:1px solid #6a55c0;background:linear-gradient(135deg,#211758,#3b246d);border-radius:16px;padding:13px}.notice-card.unread{box-shadow:0 0 20px #a65cff55}.notice-card small{color:#c7c1e8}.notice-empty{text-align:center;color:#c7c1e8;padding:18px}

.announce-carousel{display:flex;gap:12px;overflow-x:auto;scroll-snap-type:x mandatory;padding-bottom:6px}.announce-slide{flex:0 0 min(88%,520px);scroll-snap-align:start;border-radius:18px;overflow:hidden;border:1px solid #6752c6;background:#17103f;cursor:pointer}.announce-slide img{width:100%;aspect-ratio:16/10;object-fit:cover;display:block}.announce-slide .copy{padding:14px}.announce-slide .tag{display:inline-block;padding:4px 9px;border-radius:999px;background:#6e49d7;font-size:12px}.announce-slide p{max-width:none;margin:8px 0 0}.announce-detail img{width:100%;border-radius:16px;max-height:420px;object-fit:contain;background:#090622}.push-times{display:grid;grid-template-columns:repeat(3,1fr);gap:8px}.push-times span{padding:9px;border:1px solid #5a4aac;border-radius:12px;text-align:center}
</style></head><body><div class="stars"></div><div class="app"><aside class="side"><div class="logo">🌈 Rainbow Life</div><div class="nav"><button class="active" onclick="go('home')">🏠 個人中心</button><button onclick="go('calendar')">📅 行事曆</button><button onclick="action('shop')">🛒 商店</button><button onclick="action('bag')">🎒 背包</button><button onclick="action('frame')">🖼️ 頭像框</button><button onclick="action('fortune')">🔮 今日運勢</button><div class="nav-title">管理中心</div><button class="admin-menu" onclick="go('admin')">🛠️ 管理中心</button><div class="nav-title owner-menu">Rainbow Life 控制台</div><button class="owner-menu" onclick="go('owner')">👑 系統最高權限</button></div></aside><main class="main"><header class="top"><h2>👑 Rainbow Life 個人中心</h2><div class="badge" id="roleBadge">載入中</div></header><section id="home"><div class="profile-card"><div class="hero"><div class="avatar-wrap"><span class="crown" id="crown">👑</span><img class="avatar" id="avatar" alt="LINE 大頭照"></div><div class="hero-info"><h1 id="name">Rainbow</h1><div class="sub" id="title">Rainbow Life</div><div style="margin-top:12px">⭐ LV.<b id="level">1</b>　<span id="expText"></span></div><div class="progress"><i id="expBar"></i></div><div class="profile-meta"><div class="mini"><b id="rankText">彩虹 I</b><span>🌈 段位</span></div><div class="mini"><b id="zodiac">--</b><span>♈ 星座</span></div><div class="mini"><b id="regionText">未設定</b><span>📍 地區</span></div></div><div class="stats"><div class="stat coins-stat"><b id="coins">0</b><span>🌈 彩虹幣</span></div><div class="stat"><b id="tickets">0</b><span>🎟️ 抽獎券</span></div><div class="stat"><b id="vip">一般</b><span>💎 VIP</span></div><div class="stat"><b id="birthday">--</b><span>🎂 生日</span></div><div class="stat"><b id="streak">0</b><span>🔥 連續簽到</span></div></div></div><div class="latest-sign">📅 最新簽到：<span id="latestSign">尚未簽到</span></div><a class="btn profile-only-button" href="#home">👤 個人中心</a></div></div><div class="grid"><section class="card announcement"><h3>📢 最新公告</h3><div id="announcement"><p>歡迎回到 Rainbow Life</p></div></section><section class="card"><h3>✨ 今日資訊</h3><ul class="events"><li>🔮 今日運勢：<b id="fortune">尚未占卜</b></li><li>🎡 轉盤狀態：<b id="wheel">尚未完成</b></li><li>💬 今日聊天：<b id="messages">0</b></li><li>🖼️ 今日貼圖：<b id="stickers">0</b></li></ul></section></div><section class="card" style="margin-top:14px"><h3>🔔 個人通知</h3><div id="personalNotifications" class="notice-stack"></div></section><section class="card" style="margin-top:14px"><h3>⚡ 快捷功能</h3><div class="quick"><button onclick="action('shop')"><span>🛒</span>商店</button><button onclick="action('bag')"><span>🎒</span>背包</button><button onclick="action('frame')"><span>🖼️</span>頭像框</button><button onclick="action('title')"><span>🏅</span>稱號</button><button onclick="action('wheel')"><span>🎡</span>每日轉盤</button><button onclick="action('fortune')"><span>🔮</span>今日運勢</button><button onclick="go('calendar')"><span>📅</span>行事曆</button><button onclick="editProfile()"><span>⚙️</span>設定</button></div></section></section><section id="calendar" class="card" style="display:none"><h3>📅 我的行事曆</h3><button class="btn" onclick="eventModal()">新增提醒</button><ul class="events" id="eventList"></ul></section><section id="admin" class="card admin-zone"><h3>🛠️ 管理中心</h3><p class="sub">群長與管理員可管理目前群組。</p><div class="quick"><button onclick="adminAction('members')"><span>👥</span>成員管理</button><button onclick="announcementModal()"><span>📢</span>公告管理</button><button onclick="adminAction('shop')"><span>🛒</span>商店管理</button><button onclick="adminAction('frames')"><span>🖼️</span>頭像框</button><button onclick="adminAction('titles')"><span>🏅</span>稱號管理</button><button onclick="adminAction('permissions')"><span>🛡️</span>權限管理</button><button onclick="adminAction('settings')"><span>⚙️</span>系統設定</button><button onclick="adminAction('logs')"><span>📋</span>操作紀錄</button></div><div id="adminResult"></div></section><section id="owner" class="card owner-zone"><h3>👑 Rainbow Life 控制台</h3><p>系統最高權限專屬：全群組總覽、群長管理、全站公告、全站商品、系統統計與資料庫狀態。</p><div class="quick"><button onclick="ownerAction('overview')"><span>📊</span>系統總覽</button><button onclick="ownerAction('groups')"><span>🌈</span>所有群組</button><button onclick="ownerAction('leaders')"><span>👑</span>群長管理</button><button onclick="ownerAction('global')"><span>📢</span>全站公告</button></div><div id="ownerResult"></div></section></main></div><nav class="bottom"><button onclick="go('home')"><span>🏠</span>首頁</button><button onclick="action('shop')"><span>🛒</span>商店</button><button onclick="go('calendar')"><span>📅</span>行事曆</button><button onclick="go(DATA&&DATA.role==='owner'?'owner':(DATA&&DATA.role!=='member'?'admin':'home'))"><span>👤</span>我的</button></nav><div class="modal" id="modal"><div class="dialog" id="dialog"></div></div><div class="toast" id="toast"></div>
<script>
let DATA=null;const Q=location.search;function api(path,opt={}){return fetch(path+Q,{...opt,headers:{'Content-Type':'application/json',...(opt.headers||{})}}).then(async r=>{let j=await r.json();if(!r.ok)throw new Error(j.detail||'操作失敗');return j})}function esc(s){return String(s??'').replace(/[&<>"']/g,m=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[m]))}function toast(t){let e=document.getElementById('toast');e.textContent=t;e.style.display='block';setTimeout(()=>e.style.display='none',2200)}function closeModal(){document.getElementById('modal').classList.remove('show')}function go(id){['home','calendar','admin','owner'].forEach(x=>document.getElementById(x).style.display=x===id?'block':'none');if(id==='admin')loadAdmin();if(id==='owner')ownerAction('overview')}async function action(name){try{let j=await api('/api/rainbow/feature/'+name,{method:(name==='wheel'||name==='fortune')?'POST':'GET'});showFeature(name,j);if(name==='wheel'||name==='fortune'){DATA=await api('/api/rainbow/me');document.getElementById('coins').textContent=DATA.coins.toLocaleString();document.getElementById('fortune').textContent=DATA.fortune;document.getElementById('wheel').textContent=DATA.wheel_done?'已完成':'尚未完成'}}catch(e){toast(e.message)}}function showFeature(name,j){let h='<h3>'+esc(j.title||name)+'</h3>';if(j.message)h+='<p style="white-space:pre-wrap">'+esc(j.message)+'</p>';if(j.items)h+='<ul class="admin-list">'+j.items.map(x=>'<li><b>'+esc(x.name||x.title_name||x.item_name)+'</b>'+((x.price??null)!==null?'　🌈'+Number(x.price).toLocaleString():'')+(x.quantity?'　×'+x.quantity:'')+(x.description?'<br><span class="sub">'+esc(x.description)+'</span>':'')+(name==='shop'?'<br><button class="btn" onclick="buyItem('+JSON.stringify(x.name)+')">購買</button>':'')+(name==='title'?'<br><button class="btn" onclick="buyTitle('+JSON.stringify(x.title_name)+')">購買／套用</button>':'')+(name==='frame'?'<br><button class="btn" onclick="equipFrame('+JSON.stringify(x.frame_key)+')">購買／套用</button>':'')+'</li>').join('')+'</ul>';document.getElementById('dialog').innerHTML=h+'<button class="btn" onclick="closeModal()">關閉</button>';document.getElementById('modal').classList.add('show')}async function buyItem(n){try{let j=await api('/api/rainbow/shop/buy',{method:'POST',body:JSON.stringify({name:n})});toast(j.message);closeModal();DATA=await api('/api/rainbow/me');document.getElementById('coins').textContent=DATA.coins.toLocaleString()}catch(e){toast(e.message)}}async function buyTitle(n){try{let j=await api('/api/rainbow/title/buy',{method:'POST',body:JSON.stringify({name:n})});toast(j.message);closeModal();DATA=await api('/api/rainbow/me');document.getElementById('title').textContent=DATA.title;document.getElementById('coins').textContent=DATA.coins.toLocaleString()}catch(e){toast(e.message)}}async function equipFrame(n){try{let j=await api('/api/rainbow/frame/equip',{method:'POST',body:JSON.stringify({frame_key:n})});toast(j.message);closeModal()}catch(e){toast(e.message)}}function roleText(r){return {owner:'👑 Rainbow Life Owner',leader:'👑 群長',admin:'🛡️ 管理員',member:'👤 一般成員'}[r]||r}function zodiacFromBirthday(v){let m=String(v||'').match(/(\d{1,2})[-\/](\d{1,2})$/);if(!m)return '尚未設定';let md=Number(m[1])*100+Number(m[2]);return md>=1222||md<=119?'摩羯座':md<=218?'水瓶座':md<=320?'雙魚座':md<=419?'牡羊座':md<=520?'金牛座':md<=621?'雙子座':md<=722?'巨蟹座':md<=822?'獅子座':md<=922?'處女座':md<=1023?'天秤座':md<=1122?'天蠍座':'射手座'}function rankFromLevel(l){l=Number(l||1);let names=['🌱 彩虹','💙 彩虹勇者','💜 彩虹菁英','❤️ 彩虹大師','🩵 彩虹星耀','🧡 彩虹璀璨','👑 彩虹永恆'];if(l>=351)return l>=451?'♾️ 彩虹神話':l>=401?'✨ 彩虹至尊':'🌈 彩虹傳奇';let tier=Math.min(6,Math.floor((l-1)/50));let sub=Math.min(5,Math.floor(((l-1)%50)/10)+1);return names[tier]+' '+['I','II','III','IV','V'][sub-1]}async function load(){try{DATA=await api('/api/rainbow/me');document.getElementById('name').textContent=DATA.name;document.getElementById('title').textContent=DATA.title;document.getElementById('level').textContent=DATA.level;document.getElementById('coins').textContent=DATA.coins.toLocaleString();document.getElementById('tickets').textContent=DATA.tickets;document.getElementById('vip').textContent=DATA.vip?(DATA.vip_until||'啟用中'):'一般';document.getElementById('birthday').textContent=DATA.birthday;document.getElementById('zodiac').textContent=zodiacFromBirthday(DATA.birthday);document.getElementById('rankText').textContent=rankFromLevel(DATA.level);document.getElementById('regionText').textContent=DATA.region||'未設定';document.getElementById('latestSign').textContent=DATA.latest_sign_date||'尚未簽到';document.getElementById('streak').textContent=DATA.streak+'天';document.getElementById('fortune').textContent=DATA.fortune;document.getElementById('wheel').textContent=DATA.wheel_done?'已完成':'尚未完成';document.getElementById('messages').textContent=DATA.today_messages;document.getElementById('stickers').textContent=DATA.today_stickers;document.getElementById('roleBadge').textContent=roleText(DATA.role);document.getElementById('avatar').src=DATA.picture_url||'/rainbow-static/rainbow_life_boy.png';let pct=Math.min(100,Math.round(DATA.level_exp/Math.max(1,DATA.exp_needed)*100));document.getElementById('expBar').style.width=pct+'%';document.getElementById('expText').textContent=DATA.level_exp.toLocaleString()+' / '+DATA.exp_needed.toLocaleString();if(DATA.role==='member'){document.querySelectorAll('.admin-menu,.owner-menu').forEach(e=>e.style.display='none');document.getElementById('crown').style.display='none'}else{document.getElementById('admin').classList.add('show')}if(DATA.role==='owner'){document.querySelectorAll('.owner-menu').forEach(e=>e.style.display='block');document.getElementById('owner').classList.add('show')}renderAnnouncements();renderEvents();renderPersonalNotifications()}catch(e){document.body.innerHTML='<div style="padding:40px;color:white;text-align:center"><h2>無法開啟 Rainbow Life</h2><p>'+esc(e.message)+'</p><p>請回到 LINE 群組重新點選「個人中心」。</p></div>'}}function renderPersonalNotifications(){let n=DATA.notifications||[];document.getElementById('personalNotifications').innerHTML=n.length?n.map(x=>'<div class="notice-card '+(x.is_read?'':'unread')+'"><b>'+esc(x.title)+'</b><p>'+esc(x.content||'')+'</p><small>'+esc(String(x.created_at||'').replace('T',' ').slice(0,16))+'</small></div>').join(''):'<div class="notice-empty">目前沒有新通知</div>'}function renderAnnouncements(){let a=DATA.announcements||[];document.getElementById('announcement').innerHTML=a.length?'<div class="announce-carousel">'+a.map((x,i)=>'<article class="announce-slide" onclick="openAnnouncement('+i+')">'+(x.image_url?'<img src="'+esc(x.image_url)+'" alt="'+esc(x.title)+'">':'')+'<div class="copy"><span class="tag">'+esc(x.category||'公告')+'</span><h3>'+esc(x.title)+'</h3>'+(!x.image_url?'<p>'+esc(x.content).slice(0,100)+'</p>':'')+'</div></article>').join('')+'</div>':'<h2>目前沒有公告</h2>'}function openAnnouncement(i){let x=(DATA.announcements||[])[i];if(!x)return;document.getElementById('dialog').innerHTML='<div class="announce-detail">'+(x.image_url?'<img src="'+esc(x.image_url)+'">':'')+'<h2>'+esc(x.title)+'</h2><p style="white-space:pre-wrap">'+esc(x.content||'')+'</p><p class="sub">'+esc(x.start_date||'')+(x.end_date?' ～ '+esc(x.end_date):'')+'</p><button class="btn" onclick="closeModal()">返回個人中心</button></div>';document.getElementById('modal').classList.add('show')}function renderEvents(){document.getElementById('eventList').innerHTML=(DATA.events||[]).map(e=>'<li><b>'+esc(e.event_date)+'</b>　'+esc(e.title)+'<br><span class="sub">'+esc(e.note)+'</span></li>').join('')||'<li>目前沒有提醒事項</li>'}function eventModal(){document.getElementById('dialog').innerHTML='<h3>新增提醒</h3><input id="ed" type="date"><input id="et" placeholder="提醒標題"><textarea id="en" placeholder="備註"></textarea><button class="btn" onclick="saveEvent()">儲存</button> <button class="btn" onclick="closeModal()">取消</button>';document.getElementById('modal').classList.add('show')}async function saveEvent(){try{await api('/api/rainbow/calendar',{method:'POST',body:JSON.stringify({event_date:ed.value,title:et.value,note:en.value})});closeModal();toast('已儲存提醒');DATA=await api('/api/rainbow/me');renderEvents()}catch(e){toast(e.message)}}function editProfile(){document.getElementById('dialog').innerHTML='<h3>個人設定</h3><input id="bio" placeholder="自我介紹" value="'+esc(DATA.bio)+'"><input id="region" placeholder="地區" value="'+esc(DATA.region)+'"><button class="btn" onclick="saveProfile()">儲存</button> <button class="btn" onclick="closeModal()">取消</button>';document.getElementById('modal').classList.add('show')}async function saveProfile(){try{await api('/api/rainbow/profile',{method:'POST',body:JSON.stringify({bio:bio.value,region:region.value})});closeModal();toast('個人設定已儲存')}catch(e){toast(e.message)}}function announcementModal(){document.getElementById('dialog').innerHTML='<h3>新增公告／活動</h3><input id="at" placeholder="公告標題"><textarea id="ac" placeholder="完整公告內容／活動辦法"></textarea><input id="ai" placeholder="圖片網址（選填）"><input id="as" type="date" title="開始日期"><input id="ae" type="date" title="結束日期"><select id="atype" style="width:100%;padding:11px;margin:6px 0;border-radius:10px;background:#0d0928;color:white"><option value="announcement">公告</option><option value="activity">活動</option></select><label style="display:block;margin:10px 0"><input id="apush" type="checkbox"> 聊天室推播</label><div class="push-times"><span>08:00</span><span>12:00</span><span>18:00</span></div><p class="sub">推播時間固定；多則公告會合併為可左右滑動頁面。</p><button class="btn" onclick="saveAnnouncement()">發布</button> <button class="btn" onclick="closeModal()">取消</button>';document.getElementById('modal').classList.add('show')}async function saveAnnouncement(){try{await api('/api/rainbow/admin/announcement',{method:'POST',body:JSON.stringify({title:at.value,content:ac.value,image_url:ai.value,start_date:as.value,end_date:ae.value,push_to_chat:apush.checked,detail_type:atype.value,category:atype.value==='activity'?'活動':'公告'})});closeModal();toast('公告已發布');DATA=await api('/api/rainbow/me');renderAnnouncements()}catch(e){toast(e.message)}}async function loadAdmin(){if(!DATA||DATA.role==='member')return;let j=await api('/api/rainbow/admin/overview');document.getElementById('adminResult').innerHTML='<ul class="admin-list"><li>👥 成員總數：<b>'+j.members+'</b></li><li>💎 VIP：<b>'+j.vip+'</b></li><li>🛡️ 管理人員：<b>'+j.admins+'</b></li><li>💬 今日聊天：<b>'+j.today_messages+'</b></li></ul>'}async function adminAction(k){if(k==='members'){let j=await api('/api/rainbow/admin/members');document.getElementById('adminResult').innerHTML='<ul class="admin-list">'+j.items.map(x=>'<li>'+esc(x.name)+'　Lv.'+x.level+'　🌈'+x.coins.toLocaleString()+'</li>').join('')+'</ul>'}else{let j=await api('/api/rainbow/admin/'+k);document.getElementById('adminResult').innerHTML='<pre style="white-space:pre-wrap">'+esc(JSON.stringify(j,null,2))+'</pre>'}}async function ownerAction(k){if(!DATA||DATA.role!=='owner')return;let j=await api('/api/rainbow/owner/'+k);document.getElementById('ownerResult').innerHTML='<pre style="white-space:pre-wrap">'+esc(JSON.stringify(j,null,2))+'</pre>'}load();
</script></body></html>'''
...
def register_rainbow_web(app, line_bot_api):
    ...
    @app.post('/api/rainbow/internal/personal-notification')
    async def add_personal_notification(request: Request):
        p = await request.json()
        secret = str(request.headers.get('x-rainbow-internal-secret') or '')
        if not hmac.compare_digest(secret, WEB_SECRET):
            raise HTTPException(403, '內部驗證失敗。')
        gid=str(p.get('group_id') or ''); uid=str(p.get('user_id') or ''); title=str(p.get('title') or '')[:120]; content=str(p.get('content') or '')[:500]; kind=str(p.get('kind') or 'system')[:30]
        if not gid or not uid or not title: raise HTTPException(400,'缺少通知資料。')
        conn=get_connection()
        try:
            with conn.cursor() as c: c.execute('INSERT INTO web_personal_notifications(group_id,user_id,kind,title,content) VALUES(%s,%s,%s,%s,%s)',(gid,uid,kind,title,content))
            conn.commit()
        finally: conn.close()
        return {'ok':True}

    @app.post('/api/rainbow/notifications/read')
    async def read_personal_notifications(request: Request):
        uid,gid=_auth(request); conn=get_connection()
        try:
            with conn.cursor() as c: c.execute('UPDATE web_personal_notifications SET is_read=1 WHERE group_id=%s AND user_id=%s',(gid,uid))
            conn.commit()
        finally: conn.close()
        return {'ok':True}
    ...
    async def admin_announcement(request: Request):
        uid,gid,r=require_admin(request); p=await request.json(); title=str(p.get('title') or '').strip()[:100]; content=str(p.get('content') or '').strip()[:3000]
        image_url=str(p.get('image_url') or '').strip()[:1000]; category=str(p.get('category') or '公告')[:30]; start_date=str(p.get('start_date') or '')[:10]; end_date=str(p.get('end_date') or '')[:10]; push_to_chat=1 if p.get('push_to_chat') else 0; detail_type=str(p.get('detail_type') or 'announcement')[:30]
        if not title or not content: raise HTTPException(400,'請填寫公告標題與內容。')
        conn=get_connection()
        try:
            with conn.cursor() as c: c.execute('INSERT INTO web_announcements(group_id,title,content,image_url,category,start_date,end_date,push_to_chat,detail_type,created_by) VALUES(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)',(gid,title,content,image_url,category,start_date,end_date,push_to_chat,detail_type,uid))
            conn.commit()
        finally: conn.close()
        return {'ok':True}


    @app.get('/api/rainbow/internal/announcement-carousel')
    async def announcement_carousel_feed(request: Request, group_id: str = ''):
        secret=str(request.headers.get('x-rainbow-internal-secret') or '')
        if not hmac.compare_digest(secret, WEB_SECRET): raise HTTPException(403,'內部驗證失敗。')
        today=datetime.datetime.now(TZ).date().isoformat(); conn=get_connection()
        try:
            with conn.cursor() as c:
                c.execute("SELECT id,title,content,image_url,category,start_date,end_date,detail_type FROM web_announcements WHERE group_id=%s AND is_active=1 AND push_to_chat=1 AND (start_date='' OR start_date<=%s) AND (end_date='' OR end_date>=%s) ORDER BY id DESC LIMIT 10",(group_id,today,today))
                rows=[dict(x) for x in c.fetchall()]
        finally: conn.close()
        base=_public_base_url()
        cards=[]
        for x in rows:
            detail=f"{base}/player#announcement-{x['id']}" if base else ''
            cards.append({'id':x['id'],'title':x['title'],'text':x['content'][:120],'image_url':x.get('image_url') or '','category':x.get('category') or '公告','detail_url':detail,'button_label':'個人中心'})
        return {'group_id':group_id,'fixed_push_times':['08:00','12:00','18:00'],'message_count':1 if cards else 0,'carousel':cards}
