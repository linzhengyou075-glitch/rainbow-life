...
function frameArtUrl(key){if(key==='leader_glory')return '/rainbow-static/leader-frame.png?v=1';if(key==='diamond_crown')return '/rainbow-static/vip-frame.png?v=1';return '/rainbow-static/member-frame.png?v=1'}function applyFrameArtwork(){if(!DATA)return;let key=DATA.role==='owner'?'leader_glory':(DATA.vip?'diamond_crown':(DATA.equipped_frame||'rainbow_basic'));let wrap=document.getElementById('avatarWrap'),art=document.getElementById('avatarFrameArt');if(wrap&&art){wrap.className='avatar-wrap has-art frame-'+key;art.src=frameArtUrl(key)}let shell=document.getElementById('frameAvatarShell'),preview=document.getElementById('framePreviewArt');if(shell&&preview){shell.className='frame-avatar-shell has-art frame-'+key;preview.src=frameArtUrl(key)}}
