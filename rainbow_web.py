...
def register_rainbow_web(app, line_bot_api):
    ...
    async def admin_announcement(request: Request):
        ...
        try:
            with conn.cursor() as c:
                c.execute('INSERT INTO web_announcements(group_id,title,content,image_data,image_mime,link_url,created_by,sort_order) VALUES(%s,%s,%s,%s,%s,%s,%s,%s)',(gid,title,content,image_bytes,image_mime,link_url,uid,sort_order))
            conn.commit()
        finally: conn.close()
