...
/* Step 3.2 overrides */
function openAdminWeb(){location.href='/player/admin'+location.search}
...
adminAction=async function(k){let box=document.getElementById('adminResult');box.innerHTML='<p>資料載入中…</p>';if(k==='members'){let j=await api('/api/rainbow/admin/members');box.innerHTML='<h3>👥 成員管理</h3>'+adminTable(['成員','等級','VIP','今日聊天','最近簽到'],j.items.map(function(x){return '<tr><td>'+esc(x.name||'未命名')+'</td><td>Lv.'+x.level+'</td><td>'+(x.is_vip?'💎 VIP':'一般')+'</td><td>'+x.today_messages+'</td><td>'+esc(x.last_sign_in||'尚無紀錄')+'</td></tr>'}))}else if(k==='inactive'){let j=await api('/api/rainbow/admin/inactive');box.innerHTML='<h3>⚠️ 未簽到／低活躍名單</h3><div class="admin-filter"><span class="status-pill danger">未簽到 '+j.summary.unsigned+'</span><span class="status-pill warn">低活躍 '+j.summary.low_activity+'</span></div>'+adminTable(['成員','簽到','未簽到天數','今日聊天','活躍狀態','最近簽到'],j.items.map(function(x){return '<tr><td>'+esc(x.name||'未命名')+'</td><td><span class="status-pill '+(x.signed_today?'ok':'danger')+'">'+(x.signed_today?'已簽到':'未簽到')+'</span></td><td>'+x.days_without_sign+'</td><td>'+x.today_messages+'</td><td><span class="status-pill '+(x.activity_status==='正常'?'ok':x.activity_status==='注意'?'warn':'danger')+'">'+x.activity_status+'</span></td><td>'+esc(x.last_sign_in||'尚無紀錄')+'</td></tr>'}))+'<p class="sub">只供名單判定與提醒，不會自動踢人或自動禁言。</p>'}else if(k==='achievements'){let j=await api('/api/rainbow/admin/achievements');box.innerHTML='<h3>🏆 成就管理</h3><p class="sub">可建立、編輯與停用成就；頭像框不可購買。</p><div class="admin-form-grid"><input id="achKey" placeholder="成就代碼，例如 sign_100"><input id="achName" placeholder="成就名稱"><textarea class="full" id="achDescription" placeholder="成就說明"></textarea><select id="achType"><option value="sign_total">累積簽到</option><option value="messages">聊天數</option><option value="manual">手動／活動</option></select><input id="achValue" type="number" min="0" value="1" placeholder="條件數值"><input id="achFrame" placeholder="獎勵頭像框代碼（可空白）"><input id="achIcon" value="🏆" placeholder="圖示"><button class="btn full" onclick="saveAchievementAdmin()">儲存成就</button></div><div class="achievement-admin-grid" style="margin-top:14px">'+j.items.map(function(x){return '<div class="achievement-admin-card"><b>'+esc(x.icon||'🏆')+' '+esc(x.name)+'</b><p>'+esc(x.description)+'</p><small>代碼：'+esc(x.achievement_key)+'<br>條件：'+esc(x.condition_label)+'<br>獎勵框：'+esc(x.frame_name||x.frame_key||'無')+'<br>已解鎖：'+x.unlocked_count+' 人</small><div class="admin-inline-actions"><button class="mini-btn" onclick="editAchievementAdmin('+JSON.stringify(x.achievement_key)+')">編輯</button><button class="mini-btn" onclick="toggleAchievementAdmin('+JSON.stringify(x.achievement_key)+','+(x.is_active?'false':'true')+')">'+(x.is_active?'停用':'啟用')+'</button></div></div>'}).join('')+'</div>'}else if(k==='vip'){let j=await api('/api/rainbow/admin/vip');box.innerHTML='<h3>💎 VIP 管理</h3>'+adminTable(['成員','狀態','期限'],j.items.map(function(x){return '<tr><td>'+esc(x.name||'未命名')+'</td><td>'+(x.is_vip?'VIP':'一般')+'</td><td>'+esc(x.vip_until||'—')+'</td></tr>'}))}else{let j=await api('/api/rainbow/admin/'+k);box.innerHTML='<h3>'+esc(j.title||'管理功能')+'</h3><p class="sub">'+esc(j.message||'')+'</p>'}}



/* Step 3.5｜完整後台功能頁與管理操作 */
(function(){const st=document.createElement('style');st.textContent=`.s35-toolbar{display:flex;gap:8px;flex-wrap:wrap;margin:0 0 12px}.s35-search{flex:1;min-width:180px;padding:10px 12px;border-radius:12px;border:1px solid rgba(255,255,255,.14);background:rgba(6,4,35,.58);color:#fff}.s35-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:10px}.s35-card{padding:13px;border-radius:16px;background:rgba(255,255,255,.055);border:1px solid rgba(255,255,255,.11)}.s35-card b,.s35-card small{display:block}.s35-card small{color:#cec5eb;margin-top:4px}.s35-actions{display:flex;gap:6px;flex-wrap:wrap;margin-top:10px}.s35-actions button{padding:7px 9px;border:0;border-radius:10px;background:rgba(147,105,255,.23);color:#fff}.s35-actions button.danger{background:rgba(255,83,130,.2)}.s35-form{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:9px;margin-bottom:14px}.s35-form input,.s35-form textarea,.s35-form select{width:100%;padding:10px 11px;border-radius:11px;border:1px solid rgba(255,255,255,.14);background:rgba(6,4,35,.58);color:#fff}.s35-form .full{grid-column:1/-1}.s35-switch{display:flex;align-items:center;justify-content:space-between;padding:11px;border-radius:12px;background:rgba(255,255,255,.055)}.s35-empty{text-align:center;padding:22px;color:#cfc6eb}@media(max-width:650px){.s35-grid,.s35-form{grid-template-columns:1fr}.s35-form .full{grid-column:auto}}`;document.head.appendChild(st)})();
function s35Date(v){return v?String(v).replace('T',' ').slice(0,16):'—'}
function s35FilterCards(q){q=(q||'').toLowerCase();document.querySelectorAll('.s35-filter-item').forEach(e=>e.style.display=e.textContent.toLowerCase().includes(q)?'':'none')}
async function s35MemberAction(uid,action){let p={user_id:uid,action};if(action==='vip_on')p.days=Number(prompt('VIP 要增加幾天？','30')||30);if(action==='mute')p.hours=Number(prompt('禁言幾小時？','24')||24);try{await api('/api/rainbow/admin/member/control',{method:'POST',body:JSON.stringify(p)});toast('成員資料已更新');adminAction('members')}catch(e){toast(e.message)}}
async function s35Permission(remove){try{await api('/api/rainbow/admin/permission',{method:'POST',body:JSON.stringify({user_id:document.getElementById('s35PermUid').value.trim(),role:document.getElementById('s35PermRole').value,remove})});toast('權限已更新');adminAction('permissions')}catch(e){toast(e.message)}}
async function s35SaveSettings(){try{await api('/api/rainbow/admin/settings',{method:'POST',body:JSON.stringify({inactive_days:Number(document.getElementById('s35Inactive').value),low_activity_messages:Number(document.getElementById('s35Low').value),maintenance_mode:document.getElementById('s35Maintenance').checked})});toast('系統設定已儲存');adminAction('settings')}catch(e){toast(e.message)}}
async function s35CreateAnnouncement(){let f=document.getElementById('s35AnnImage'),image_data='';if(f.files&&f.files[0])image_data=await new Promise((ok,no)=>{let r=new FileReader();r.onload=()=>ok(r.result);r.onerror=no;r.readAsDataURL(f.files[0])});try{await api('/api/rainbow/admin/announcement',{method:'POST',body:JSON.stringify({title:document.getElementById('s35AnnTitle').value,content:document.getElementById('s35AnnContent').value,link_url:document.getElementById('s35AnnLink').value,image_data})});toast('公告已發布');adminAction('activities')}catch(e){toast(e.message)}}
async function s35AnnStatus(id,is_active){try{await api('/api/rainbow/admin/announcement/status',{method:'POST',body:JSON.stringify({id,is_active})});toast('公告狀態已更新');adminAction('activities')}catch(e){toast(e.message)}}
async function s35AnnDelete(id){if(!confirm('確定刪除這則公告？'))return;try{await api('/api/rainbow/admin/announcement/'+id,{method:'DELETE'});toast('公告已刪除');adminAction('activities')}catch(e){toast(e.message)}}
const s35OldAdminAction=adminAction;
adminAction=async function(k){let box=document.getElementById('adminResult');if(!box)return;box.innerHTML='<p>資料載入中…</p>';try{if(k==='members'){let j=await api('/api/rainbow/admin/members');box.innerHTML='<div class="s35-toolbar"><input class="s35-search" placeholder="搜尋成員" oninput="s35FilterCards(this.value)"><span class="status-pill ok">共 '+j.items.length+' 人</span></div><div class="s35-grid">'+j.items.map(x=>'<article class="s35-card s35-filter-item"><b>'+esc(x.name||'未命名')+'</b><small>Lv.'+Number(x.level||1)+'・'+(x.is_vip?'💎 VIP':'一般會員')+(x.admin_role?'・🛡️ '+esc(x.admin_role):'')+'</small><small>今日聊天 '+Number(x.today_messages||0)+'・最近簽到 '+esc(s35Date(x.last_sign_in))+'</small><small>'+(x.is_muted?'🔇 禁言中':'✅ 功能正常')+'</small><div class="s35-actions"><button onclick="s35MemberAction('+JSON.stringify(x.user_id)+','vip_on')">增加 VIP</button>'+(x.is_vip?'<button onclick="s35MemberAction('+JSON.stringify(x.user_id)+','vip_off')">取消 VIP</button>':'')+'<button class="danger" onclick="s35MemberAction('+JSON.stringify(x.user_id)+','+(x.is_muted?''unmute'':''mute'')+')">'+(x.is_muted?'解除禁言':'禁言')+'</button></div></article>').join('')+'</div>'}else if(k==='permissions'){let j=await api('/api/rainbow/admin/permissions');box.innerHTML='<div class="s35-form"><input id="s35PermUid" class="full" placeholder="成員 LINE User ID"><select id="s35PermRole"><option value="admin">管理員</option><option value="manager">進階管理員</option><option value="leader">群長</option></select><div class="s35-actions"><button onclick="s35Permission(false)">授予／更新權限</button><button class="danger" onclick="s35Permission(true)">移除權限</button></div></div><div class="s35-grid">'+(j.items||[]).map(x=>'<article class="s35-card"><b>🛡️ '+esc(x.role||'admin')+'</b><small>'+esc(x.user_id)+'</small></article>').join('')+'</div><p class="sub">只有 Owner 能調整管理權限。</p>'}else if(k==='activities'){let j=await api('/api/rainbow/admin/announcements');box.innerHTML='<div class="s35-form"><input id="s35AnnTitle" placeholder="公告標題"><input id="s35AnnLink" placeholder="活動連結（可空白）"><textarea id="s35AnnContent" class="full" placeholder="公告內容"></textarea><input id="s35AnnImage" class="full" type="file" accept="image/jpeg,image/png,image/webp,image/gif"><button class="btn full" onclick="s35CreateAnnouncement()">發布公告</button></div><button class="mini-btn" onclick="openActivityManager()">🎉 開啟活動顯示設定</button><div class="s35-grid" style="margin-top:12px">'+(j.items.length?j.items.map(x=>'<article class="s35-card"><b>'+esc(x.title)+'</b><small>'+esc(x.content||'無文字內容')+'</small><small>'+esc(x.created_at||'')+(x.has_image?'・含圖片':'')+'</small><div class="s35-actions"><button onclick="s35AnnStatus('+x.id+','+(!x.is_active)+')">'+(x.is_active?'暫停顯示':'重新啟用')+'</button><button class="danger" onclick="s35AnnDelete('+x.id+')">刪除</button></div></article>').join(''):'<div class="s35-empty">尚未建立公告</div>')+'</div>'}else if(k==='settings'){let j=await api('/api/rainbow/admin/settings'),x=j.item||{};box.innerHTML='<div class="s35-form"><label>未簽到判定天數<input id="s35Inactive" type="number" min="1" value="'+Number(x.inactive_days||7)+'"></label><label>低活躍聊天門檻<input id="s35Low" type="number" min="0" value="'+Number(x.low_activity_messages||3)+'"></label><label class="s35-switch full"><span><b>維護模式</b><small>供網站其他流程判斷</small></span><input id="s35Maintenance" type="checkbox" '+(x.maintenance_mode?'checked':'')+'></label><div class="s35-card full"><b>🌈 Rainbow Life 彩虹星空主題</b><small>永久固定，不提供主題、顏色或動畫切換。</small></div><button class="btn full" onclick="s35SaveSettings()">儲存系統設定</button></div>'}else if(k==='logs'){let j=await api('/api/rainbow/admin/logs');box.innerHTML='<div class="s35-toolbar"><input class="s35-search" placeholder="搜尋操作紀錄" oninput="s35FilterCards(this.value)"></div><div class="s35-grid">'+((j.items||[]).length?(j.items||[]).map(x=>'<article class="s35-card s35-filter-item"><b>📋 '+esc(x.action||'system')+'</b><small>'+esc(x.detail||'')+'</small><small>'+esc(s35Date(x.created_at))+'</small></article>').join(''):'<div class="s35-empty">目前沒有操作紀錄</div>')+'</div>'}else await s35OldAdminAction(k)}catch(e){box.innerHTML='<div class="s35-empty">'+esc(e.message||'資料載入失敗')+'</div>'}}

/* Step 3.4｜最高權限後台首頁互動 */
let STEP34_MENU_PAGE=0;
function step34Move(){let t=document.getElementById('step34MenuTrack');if(t)t.style.transform=`translateX(-${STEP34_MENU_PAGE*100}%)`;document.querySelectorAll('#step34MenuDots i').forEach((d,i)=>d.classList.toggle('active',i===STEP34_MENU_PAGE))}
...
function step331Setup(){
  ...
  style.textContent=`
  #home.step331-ready>*:not(.version-mark):not(#step331CompactHome){display:none!important}
...
function step331Render(){if(!DATA)return;const $=id=>document.getElementById(id);let avatar=$('s331Avatar');if(avatar)avatar.src=DATA.picture_url||'/rainbow-static/rainbow_life_boy.png';if($('s331Name'))$('s331Name').textContent=DATA.name||'Rainbow';if($('s331Role'))$('s331Role').textContent=roleText(DATA.role||'member');if($('s331Level'))$('s331Level').textContent='⭐ LV.'+(DATA.level||1);if($('s331Vip'))$('s331Vip').textContent=DATA.vip?'💎 VIP '+(DATA.vip_until||'永久'):'一般會員';if($('s331Title'))$('s331Title').textContent='🏷️ '+(DATA.title||'彩虹旅人');let percent=Number(DATA.level_percent??DATA.exp_percent??0);if(!percent&&DATA.exp_next)percent=Math.min(100,Math.round(Number(DATA.exp||0)/Number(DATA.exp_next||1)*100));if($('s331Exp'))$('s331Exp').style.width=Math.max(0,Math.min(100,percent))+'%';
 ...
 if(admin)pages.push([['👑','管理後台','Owner／管理權限',"openAdminWeb()",true],['📢','公告管理','發布與管理公告',"openAdminWeb()",true],['👥','成員管理','成員與活躍狀態',"openAdminWeb()",true],['⚙️','系統設定','維護與權限設定',"openAdminWeb()",true]]);
...
function step331OpenActivities(){let el=document.getElementById('lifePushCenter');if(el){homeRestoreSection(el,'🌈 活動中心')}else unavailable('活動中心')}
...
setTimeout(step331Setup,0);setTimeout(step331Render,900);
</script></body></html>'''
...
def register_rainbow_web(app, line_bot_api):
    ...
    async def player_admin_page(request: Request):
        ...
        return HTMLResponse(_dashboard_html().replace('</body>','<script>document.addEventListener(\'DOMContentLoaded\',()=>setTimeout(()=>go(\'admin\'),120));</script></body>'))
