...
def register_rainbow_web(app, line_bot_api):
    ...
    async def admin_members(request: Request):
        ...
        try:
            with conn.cursor() as c:
                c.execute('SELECT user_id,name,level,is_vip,vip_until,today_msg_count AS today_messages,last_sign_in FROM players WHERE group_id=%s ORDER BY level DESC,exp DESC LIMIT 200',(gid,)); rows=c.fetchall()
        finally: conn.close()
