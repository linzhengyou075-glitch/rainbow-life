...
.avatar-wrap .avatar-frame-art{position:absolute;z-index:7;width:270px;height:270px;max-width:none;object-fit:contain;pointer-events:none;filter:drop-shadow(0 0 12px rgba(181,115,255,.72));animation:framePulse 3.2s ease-in-out infinite}
...
function frameArtUrl(key){if(key==='leader_glory')return '/rainbow-static/leader-frame.png?v=2';if(key==='diamond_crown')return '/rainbow-static/vip-frame.png?v=2';return '/rainbow-static/member-frame.png?v=2'}function applyFrameArtwork(){if(!DATA)return;let key=DATA.role==='owner'?'leader_glory':(DATA.vip?'diamond_crown':(DATA.equipped_frame||'rainbow_basic'));let wrap=document.getElementById('avatarWrap'),art=document.getElementById('avatarFrameArt');if(wrap&&art){wrap.className='avatar-wrap has-art frame-'+key;art.src=frameArtUrl(key)}let shell=document.getElementById('frameAvatarShell'),preview=document.getElementById('framePreviewArt');if(shell&&preview){shell.className='frame-avatar-shell has-art frame-'+key;preview.src=frameArtUrl(key)}}
