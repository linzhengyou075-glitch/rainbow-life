...
def _player_data(line_bot_api, group_id, user_id):
    ...
    return {
        ...
        'birthday': str(p.get('birthday') or '尚未設定'), 'zodiac': _zodiac_from_birthday(str(p.get('birthday') or '')), 'streak': int(p.get('streak_count') or 0),
...
def _zodiac_from_birthday(value):
    try:
        ...
        md = month * 100 + day
        ranges = [
            (120, '摩羯座'), (219, '水瓶座'), (321, '雙魚座'), (420, '牡羊座'),
            (521, '金牛座'), (622, '雙子座'), (723, '巨蟹座'), (823, '獅子座'),
            (923, '處女座'), (1024, '天秤座'), (1123, '天蠍座'), (1222, '射手座'),
            (1232, '摩羯座')
        ]
...
def register_rainbow_web(app, line_bot_api):
    ...
    async def api_profile(request: Request):
        ...
        bio = str(payload.get('bio') or '')[:300]; region = str(payload.get('region') or '')[:80]; birthday = str(payload.get('birthday') or '').strip()[:10]
        conn = get_connection()
        try:
            with conn.cursor() as c:
                if birthday:
                    c.execute('UPDATE players SET birthday=%s WHERE group_id=%s AND user_id=%s',(birthday,gid,uid))
